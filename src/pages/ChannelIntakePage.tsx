...
const ChannelIntakePage = ({ embedded = false, formOpen, onFormOpenChange }: ChannelIntakePageProps = {}) => {
  ...
  const saveStatus = async () => {
    ...
    // activity_logs 기록 (inquiries는 leads FK 없으므로 lead_id=null)
    {
      const staffId = user?.id ?? '';
      const staffName = user?.user_metadata?.display_name ?? user?.email ?? 'unknown';
      if (staffId) {
        supabase.from('activity_logs').insert({
          lead_id: null, sales_record_id: null,
          staff_id: staffId, staff_name: staffName, store_id: null,
          channel: 'other', action_type: 'status_changed', result_type: null,
          previous_status: prevStatus ?? null, next_status: editStatus,
          memo: null, fail_reason: null, next_action_at: null,
          is_counted: false, not_counted_reason: '기타인입(inquiries) 상태변경',
          corrected_log_id: null, device_info: null, ip_address: null,
          created_by: staffId,
        }).then(({ error: e }) => {
          if (e) console.warn('[ChannelIntakePage saveStatus activity_log 실패]', e.message);
        });
      }
    }
    ...
    // activity_logs 기록 (inquiries는 leads FK 없으므로 lead_id=null)
    {
      ...
      if (staffId) {
        supabase.from('activity_logs').insert({
          ...
          staff_id: staffId, staff_name: staffName, store_id: null,
          channel: 'other', action_type: 'status_changed', result_type: null,
          previous_status: prevStatus ?? null, next_status: next,
          memo: null, fail_reason: null, next_action_at: null,
          is_counted: false, not_counted_reason: '기타인입(inquiries) 상태변경',
          corrected_log_id: null, device_info: null, ip_address: null,
          created_by: staffId,
        }).then(({ error: e }) => {
          if (e) console.warn('[ChannelIntakePage quickChangeStatus activity_log 실패]', e.message);
        });
      }
    }
    refreshOne(row.id);
