...
// ── 2ND (워치 · 인터넷 · TV프리) 통계 ─────────────────────────────
// 확정 스펙시트(확정 목록)에서 입력한 값을, 지금 대시보드에
...
function BundleStatsSection({ rows }: { rows: ConfirmedRow[] }) {
  ...
  return (
    <SectionCard title="2ND (워치 · 인터넷 · TV프리) 통계" rightSlot={<span className="text-xs text-gray-400">현재 필터 기준</span>}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* 워치 */}
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-bold text-gray-800">⌚ 워치</h3>
          ...
          ) : <div className="text-sm text-gray-400 text-center py-4">워치 입력 건이 없습니다</div>}
        </div>

        {/* 인터넷 */}
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-bold text-gray-800">🌐 인터넷</h3>
          ...
          ) : <div className="text-sm text-gray-400 text-center py-4">인터넷 입력 건이 없습니다</div>}
        </div>

        {/* TV프리 */}
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-bold text-gray-800">📺 TV프리</h3>
          ...
          ) : <div className="text-sm text-gray-400 text-center py-4">TV프리 입력 건이 없습니다</div>}
        </div>
      </div>
    </SectionCard>
  );
...
export default function ConfirmedDashboardPage() {
  ...
  // (2ND 워치·인터넷·TV프리 통계는 전체 확정 파이프라인 추적용이라 택배발송 포함 그대로 둔다.)
        ...
        rightSlot={
          <>
            ...
            <Button
              ...
              onClick={() => {
                if (rows.length === 0) return toast.error('내보낼 데이터가 없습니다');
                downloadMnpSplitReportXls(rows);
