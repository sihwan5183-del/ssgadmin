...
export function LeadsRealtimeNotifier() {
  ...
  useEffect(() => {
    ...
    const handle = (row: LeadRow) => {
      ...
      const tab = isDogmaru ? "dogmaru" : "meta";
      const displayName =
        (isDogmaru ? row.customer_name : row.name) || row.name || row.customer_name || "(이름 미상)";
      const displayPhone = maskPhone(
        (isDogmaru ? row.customer_phone : row.phone) || row.phone || row.customer_phone,
      );
      const channelLabel = isDogmaru ? "🐶 [도그마루] 제휴 인입건이 등록되었습니다!" : "📢 [메타광고] 신규 잠재고객이 등록되었습니다!";
      ...
      toast.custom(
        (id) => (
          <div
            ...
            onClick={() => {
              toast.dismiss(id);
              navigate(`/leads?tab=${tab}&highlight=${row.id}`);
            }}
          ...
          >
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-1.5 text-[11px] text-slate-500">클릭하면 해당 탭으로 이동합니다</div>
            </div>
            <button
              type="button"
              aria-label="알림 닫기"
